// components/PrivateChatToggle.tsx
...
export function PrivateChatToggle({ isPrivate, onToggle, disabled = false }: PrivateChatToggleProps) {
  return (
    <button
      onClick={() => onToggle(!isPrivate)}
      disabled={disabled}
      className={`p-2 rounded-lg transition-all duration-200 hover:scale-105 glass border relative group ${
        isPrivate 
    ...
    >
      <div className="relative">
        ...
        {/* Check Mark Overlay when Active */}
        {isPrivate && (
      ...
      <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">
        {isPrivate ? "Disable private mode" : "Enable private mode"}
      </div>
    </button>
  )
}