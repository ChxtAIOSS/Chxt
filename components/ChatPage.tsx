    ...
    >
      ...
      <header className="glass px-6 py-3 flex items-center justify-between border-b border-white/5 flex-shrink-0">
        <div className="flex items-center space-x-4">
          {/* Single Sidebar toggle button with tooltip */}
          {setSidebarOpen && (
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-2 hover:bg-white/5 rounded-lg transition-all duration-200 hover:scale-105 text-gray-400 hover:text-gray-200 group relative"
            >
              <svg 
                ...
                className={`transition-colors ${!sidebarOpen ? 'rotate-180' : ''}`}
              >
